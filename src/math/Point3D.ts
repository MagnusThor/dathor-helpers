...
/**
 * Represents a point or vector in 3D space, extending Point2D with z-coordinate functionality.
 * Provides methods for various 3D geometric operations including rotations, vector operations,
 * and projections.
 * 
 * @class
 * @extends Point2D
 ...
 * point.rotateX(45); // Rotates the point 45 degrees around X axis
 * point.normalize(); // Converts the point to a unit vector
 * ```
 * 
...
*/
    ...
    angle(bvector: Point3D): number {
        const anorm = new Point3D(this.x, this.y, this.z);
        anorm.normalize();
        const bnorm = new Point3D(bvector.x, bvector.y, bvector.z);
        bnorm.normalize();
    ...
    cross(vectorB: Point3D): void {
        const tempvec = new Point3D(this.x, this.y, this.z);
